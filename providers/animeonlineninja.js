...
// Si Nuvio no muestra ajustes del scraper, editá acá y subí el archivo a tu repo.
// CF_CLEARANCE: pegá SOLO el valor (sin "cf_clearance="), desde Chrome → F12 → Cookies
var CF_CLEARANCE = "zLrnjeBj9XcdjQp7U6uRvvvJ1Bt5UL1b5aIPvcML2gY-1783232108-1.2.1.1-Pe4PF9CAPM_Wve6mYCHzQbS3yBqGGfGTuZ2NXeM5cjwxjnGGY7eal3WsT3_MQMWrEATJi8SrYA8ybz_LerCIBI4q58L6zRuE4too2g3_HMK.BpGaxwfdcmhh8GYirrczoh4giXGJqan95ufSKcbVwO2.vZ1HbdfD.nAXGIjOs6f1vHao5MsBUNN2zG2iA9873Q3UzcjbaxLfVEDb8wdaFfNZfmwg9HM03ftNgEiR2Ia.tXAtHdI9QKv4DrpA6ML5xHXgV.Fwg7Rjd7gLun1sJJkJ2KjmYMLuVpFDkyNxFz59X9b7o_9.QziI5Okjr3JVmaDMeWmqqvoyNJnENDGVmRrRcoKaecae7BgM9JvvRO1ogaKX_DPfoyBit1KchUrCCC.L8cu7c4NlNcT0sQ4aja5GgCMAg5ZvRJ2b.j69rvnZMGyypt3LgYSxv5yfQHni";
// PROXY_URL: Cloudflare Worker (ver workers/animeonline-proxy.js)
// Ejemplo: "https://animeonline-proxy.TU-USUARIO.workers.dev/?url="
var PROXY_URL = "";
// ==========================================
var HEADERS = {
...
function getSettings() {
  ...
  return {
    cfClearance: settings.cfClearance || CF_CLEARANCE || "",
    proxyUrl: settings.proxyUrl || settings.animeonlineProxy || PROXY_URL || "",
    animeonlineCookie: settings.animeonlineCookie || "",
...
function buildRequestHeaders(extra) {
  ...
  var cookies = [];
  if (settings.cfClearance) cookies.push("cf_clearance=" + settings.cfClearance);
  if (settings.animeonlineCookie) cookies.push(settings.animeonlineCookie);
  if (settings.cookie) cookies.push(settings.cookie);
  if (cookies.length) headers.Cookie = cookies.join("; ");
  return headers;
}

function resolveFetchUrl(url, useProxy) {
  if (!useProxy) return url;
  var settings = getSettings();
  var proxy = settings.proxyUrl || settings.animeonlineProxy;
  if (!proxy) return url;
  if (proxy.indexOf("?") >= 0) return proxy + encodeURIComponent(url);
  return proxy.replace(/\/$/, "") + "/" + encodeURIComponent(url);
}

function fetchText(url, extraHeaders, options) {
  return __async(null, null, function* () {
    ...
    if (options.useProxy === false) {
      ...
      attempts.push(true);
    } else if (settings.proxyUrl || settings.animeonlineProxy) {
      attempts.push(true);
      attempts.push(false);
    } else {
      attempts.push(false);
    ...
    for (var i = 0; i < attempts.length; i++) {
      try {
        var targetUrl = resolveFetchUrl(url, attempts[i]);
        var response = yield fetch(targetUrl, {
        ...
        lastError = e;
        console.log("[AnimeOnline] Fetch " + (attempts[i] ? "proxy" : "direct") + " failed: " + e.message);
      }
    }
...
function fetchJson(url, extraHeaders, options) {
  return __async(null, null, function* () {
    var text = yield fetchText(url, Object.assign({}, extraHeaders || {}, {
...
function buildSlugCandidates(mediaInfo, tmdbId, matchHref) {
  ...
  add(mediaInfo.malTitle);
  add(mediaInfo.malTitleEnglish);
  add(mediaInfo.malTitleJapanese);
...
function buildSearchQueries(mediaInfo) {
  ...
  if (mediaInfo.malTitleEnglish && mediaInfo.malTitleEnglish.indexOf(":") >= 0) {
    add(mediaInfo.malTitleEnglish.split(":")[0]);
  }
  if (mediaInfo.alternativeTitles) {
    for (var i = 0; i < mediaInfo.alternativeTitles.length; i++) {
      add(mediaInfo.alternativeTitles[i]);
...
function resolveMalMapping(imdbId, season, episode) {
  return __async(null, null, function* () {
    try {
      var url = "https://id-mapping-api-malid.hf.space/api/resolve?id=" + encodeURIComponent(imdbId) +
        "&s=" + encodeURIComponent(season || 1) + "&e=" + encodeURIComponent(episode || 1);
      var res = yield fetch(url, { headers: { "User-Agent": USER_AGENT } });
      if (!res.ok) return null;
...
function getMalAnimeInfo(malId) {
  return __async(null, null, function* () {
    try {
      var res = yield fetch("https://api.jikan.moe/v4/anime/" + malId, {
...
function enrichWithMalData(mediaInfo, tmdbId, mediaType, season, episode) {
  return __async(null, null, function* () {
    if (mediaType !== "tv") return mediaInfo;
    var imdbId = yield getImdbId(tmdbId, mediaType);
    if (!imdbId) return mediaInfo;
    var mapping = yield resolveMalMapping(imdbId, season, episode);
    if (!mapping || !mapping.mal_id) return mediaInfo;
    mediaInfo.malId = mapping.mal_id;
    mediaInfo.mappedEpisode = mapping.mal_episode || parseInt(episode, 10) || 1;
    console.log("[AnimeOnline] MAL: id=" + mapping.mal_id + " episode=" + mediaInfo.mappedEpisode);
    var malInfo = yield getMalAnimeInfo(mapping.mal_id);
    if (malInfo) {
      mediaInfo.malTitle = malInfo.title;
      mediaInfo.malTitleEnglish = malInfo.titleEnglish;
      mediaInfo.malTitleJapanese = malInfo.titleJapanese;
      if (!mediaInfo.alternativeTitles) mediaInfo.alternativeTitles = [];
      if (malInfo.titleSynonyms) {
        mediaInfo.alternativeTitles = mediaInfo.alternativeTitles.concat(malInfo.titleSynonyms);
      }
      if (malInfo.titleEnglish) mediaInfo.alternativeTitles.push(malInfo.titleEnglish);
      if (malInfo.title) mediaInfo.alternativeTitles.push(malInfo.title);
    }
    return mediaInfo;
  });
}
...
function scoreMatch(mediaInfo, result, mediaType) {
  var titlesToCompare = [
    ...
    mediaInfo.malTitle, mediaInfo.malTitleEnglish, mediaInfo.malTitleJapanese
  ];
  if (mediaInfo.alternativeTitles) {
...
function resolveEpisodePage(mediaInfo, tmdbId, mediaType, season, episode, match) {
  return __async(null, null, function* () {
    var episodeNum = mediaInfo.mappedEpisode || parseInt(episode, 10) || 1;
    var pageUrl = null;
    var pageHtml = null;
    ...
    if (match && match.href.indexOf("/serie/") >= 0) {
      try {
        ...
        var fromList = findEpisodeUrl(episodes, season, episodeNum);
        if (fromList) {
          pageUrl = fromList;
          pageHtml = yield fetchText(pageUrl);
    ...
    console.log("[AnimeOnline] Trying slug URLs for ep " + episodeNum + " (global " + globalEpisode + "): " + slugs.join(", "));

    var allUrls = [];
    for (var s = 0; s < slugs.length; s++) {
      var candidates = buildEpisodeUrlCandidates(slugs[s], season, episodeNum, globalEpisode);
...
function getStreams(tmdbId, mediaType, season, episode) {
  return __async(null, null, function* () {
    ...
    try {
      ...
      mediaInfo = yield enrichWithMalData(mediaInfo, tmdbId, mediaType, season, episode);
      console.log('[AnimeOnline] Searching for: "' + mediaInfo.title + '" / "' + (mediaInfo.malTitleEnglish || mediaInfo.originalTitle) + '" (' + mediaInfo.year + ")");
      ...
      if (!match) {
        ...
        if (match) {
          console.log('[AnimeOnline] Search match: "' + match.title + '" -> ' + match.href);
        }
      }

      var page = yield resolveEpisodePage(mediaInfo, tmdbId, mediaType, season, episode, match);
      if (!page) {
        console.log("[AnimeOnline] Could not resolve episode page.");
        return [];
      }
...
function onSettings() {
  return __async(null, null, function* () {
    return [
      { type: "header", label: "AnimeOnline.Ninja" },
      {
        type: "text",
        key: "proxyUrl",
        label: "Proxy URL (opcional)",
        description: "URL de proxy tipo https://tu-proxy.workers.dev/?url= para saltar Cloudflare. Igual que AnimePahe.",
        defaultValue: ""
      },
      {
        type: "text",
        key: "cfClearance",
        label: "Cookie cf_clearance (opcional)",
        description: "Pegá el valor de cf_clearance de tu navegador si el sitio te pide verificación.",
        defaultValue: ""
      }
    ];
  });
